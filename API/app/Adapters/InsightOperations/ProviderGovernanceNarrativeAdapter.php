    ...
    GovernanceNarrativePortInterface
{
    ...
    public function generate(
        ...
        $response = $this->client->narrate(
            new GovernanceNarrativeRequest(
    ...
    private function provenance(
        ...
        array $payload,
        ?string $actorId = null,
    ): AiArtifactProvenanceDto {
        $provenance = is_array($response["provenance"] ?? null)
            ? $response["provenance"]
            : [];

        return new AiArtifactProvenanceDto(
            providerName: $this->stringOrNull(
                $provenance["provider_name"] ?? null,
            ) ?? $this->providerName(),
            endpointGroup: $this->stringOrNull(
                $provenance["endpoint_group"] ?? null,
            ) ?? AiStatusSchema::ENDPOINT_GROUP_GOVERNANCE,
            model: $this->stringOrNull($provenance["model"] ?? null),
            promptVersion: $this->stringOrNull(
                $provenance["prompt_version"] ?? null,
            ),
            providerRequestId: $this->stringOrNull(
                $provenance["provider_request_id"] ?? null,
            ),
            inputHash: $this->stringOrNull($provenance["input_hash"] ?? null),
            outputHash: $this->stringOrNull(
                $provenance["output_hash"] ?? null,
            ) ?? $this->payloadHash($payload),
            latencyMs: is_numeric($provenance["latency_ms"] ?? null)
                ? (int) $provenance["latency_ms"]
                : null,
            generatedAt: $this->stringOrNull(
                $provenance["generated_at"] ?? null,
    ...
    private function vendorIdFromFacts(array $facts): string
    {
        $vendorId =
            $facts["vendor_id"] ??
            ($facts["vendor_id_hash"] ?? "vendor_governance");
