...
final readonly class RiskInsightFactsAdapter implements RiskInsightFactsPortInterface
{
    public function factsForRfq(string $tenantId, string $rfqId): RiskInsightFactsDto
    {
        ...
        usort($items, static function (array $left, array $right): int {
            ...
            return [$severityRank[$left['severity']] ?? 4, $left['title']]
                <=> [$severityRank[$right['severity']] ?? 4, $right['title']];
        });

        return new RiskInsightFactsDto($rfqId, $items, $this->manualReview($items));
    }
        ...
        foreach ([
            ...
            if ($date->isPast()) {
                $items[] = $this->riskItem(
                    ...
                    details: ['field' => $field, 'due_at' => $date->toAtomString(), 'days_overdue' => $date->diffInDays($now)],
                );
                continue;
            }
            ...
            if ($date->lessThanOrEqualTo($now->addDays(3))) {
                $items[] = $this->riskItem(
                    ...
                    source: 'rfq_schedule',
                    sourceId: (string) $rfq->id,
                    details: ['field' => $field, 'due_at' => $date->toAtomString(), 'days_remaining' => $now->diffInDays($date)],
                );
            }
        }
     ...
     * @return list<array<string, mixed>>
     */
    private function vendorFindingRiskItems(string $tenantId, string $rfqId): array
    {
        ...
        return VendorFinding::query()
            ...
            ->get()
            ->map(fn (VendorFinding $finding): array => $this->riskItem(
                domain: (string) $finding->domain,
                severity: (string) $finding->severity,
                status: (string) $finding->status,
                title: 'Vendor finding: ' . str_replace('_', ' ', (string) $finding->issue_type),
     ...
     * @return list<array<string, mixed>>
     */
    private function quoteReadinessRiskItems(string $tenantId, string $rfqId): array
    {
        return QuoteSubmission::query()
            ...
            ->orderBy('vendor_name')
            ->get()
            ->map(fn (QuoteSubmission $submission): array => $this->riskItem(
                domain: 'risk',
                ...
                details: [
                    ...
                    'warnings_count' => max((int) ($submission->warnings_count ?? 0), 0),
                    'errors_count' => max((int) ($submission->errors_count ?? 0), 0),
                ],
            ))
            ->values()
            ->all();
    ...
    private function vendorIdsForRfq(string $tenantId, string $rfqId): array
    {
        ...
        $ids = $ids->merge(RequisitionSelectedVendor::query()
            ->where('tenant_id', $tenantId)
            ->where('rfq_id', $rfqId)
            ->pluck('vendor_id'));
        $ids = $ids->merge(VendorInvitation::query()
            ->where('tenant_id', $tenantId)
            ->where('rfq_id', $rfqId)
            ->whereNotNull('vendor_id')
            ->pluck('vendor_id'));
        $ids = $ids->merge(QuoteSubmission::query()
            ->where('tenant_id', $tenantId)
            ->where('rfq_id', $rfqId)
            ->whereNotNull('vendor_id')
            ->pluck('vendor_id'));
    ...
    {
        if ((string) $submission->status === 'failed' || (int) ($submission->errors_count ?? 0) > 0) {
            return 'high';
        }

        return (string) $submission->status === 'needs_review' ? 'medium' : 'low';
    }
     ...
     */
    private function riskItem(string $domain, string $severity, string $status, string $title, string $source, string $sourceId, array $details): array
    {
        return [
            'domain' => $domain,
            'severity' => $severity,
