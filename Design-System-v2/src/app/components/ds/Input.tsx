...
export function FieldLabel({ htmlFor, required, children, className = '' }: LabelProps) {
  return (
    <label
      htmlFor={htmlFor}
      className={['block text-xs font-medium text-slate-600 mb-1', className].join(' ')}
    >
      {children}
...
export function TextInput({
  label, hint, error, prefixIcon, suffixIcon,
...
export function SearchInput({
  shortcut, onClear, containerClassName = '', compact = false, value, className = '', ...props
}: SearchInputProps) {
  return (
...
export function SelectInput({
  label, hint, error, options, placeholder, containerClassName = '',
  compact = false, className = '', id, required, ...props
}: SelectInputProps) {
  const inputId = id ?? (label ? label.toLowerCase().replace(/\s+/g, '-') : undefined);
...
export function Textarea({ label, hint, error, containerClassName = '', className = '', id, required, ...props }: TextareaProps) {
  const inputId = id ?? (label ? label.toLowerCase().replace(/\s+/g, '-') : undefined);
