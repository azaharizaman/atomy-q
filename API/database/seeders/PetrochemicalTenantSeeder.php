...
final class PetrochemicalTenantSeeder extends Seeder
{
    ...
    private function seedTenant(): void
    {
        ...
        if ($existing !== null) {
            ...
            $this->command->info(
                "Tenant NORDFJORD already exists, using existing.",
            );
            return;
        }
        ...
        if ($existingById !== null) {
            ...
            $this->command->info(
                "Tenant with default ID already exists, using existing.",
            );
            return;
        }
    ...
    private function seedProjectsAndAcl(): void
    {
        $projects = [
            [
                "name" => "Steam Cracker TAR-2026 (Rafnes)",
                "client" => "NRF-RAFNES",
                "status" => "active",
            ...
            [
                "name" => "Catalyst Regeneration Campaign Q2",
                "client" => "CAT-VENDOR-X",
                "status" => "planning",
            ...
            [
                "name" => "Stord Marine Terminal Expansion",
                "client" => "TERM-STORD",
                "status" => "active",
                "budget_type" => "fixed_price",
                "pct" => 55.0,
            ...
            [
                "name" => "Hydrogen Plant Feed Purification",
                "client" => "H2-NOR",
                "status" => "planning",
                "budget_type" => "time_and_materials",
                "pct" => 8.0,
            ...
            [
                "name" => "Lab Specialty Solvents — Frame 2024–27",
                "client" => "LAB-NFC",
                "status" => "completed",
                "budget_type" => "fixed_price",
                "pct" => 100.0,
            ...
            [
                "name" => "Flare Tip Replacement Package",
                "client" => "SAFETY-NFC",
                "status" => "on_hold",
                "budget_type" => "fixed_price",
                "pct" => 22.0,
            ...
            [
                "name" => "Sulfur Unit Pit Lining (deferred)",
                "client" => "SUL-LEG",
                "status" => "cancelled",
                "budget_type" => "fixed_price",
                "pct" => 0.0,
            ...
            [
                "name" => "Cooling Water Chemical Program",
                "client" => "UTIL-CW",
                "status" => "active",
                "budget_type" => "fixed_price",
                "pct" => 67.0,
            ...
            [
                "name" => "Berth 4 Loading Arm Renewal",
                "client" => "MARINE-JETTY",
                "status" => "active",
                "budget_type" => "fixed_price",
                "pct" => 71.0,
            ...
            [
                "name" => "Packaged Boiler House Revamp",
                "client" => "UTIL-STM",
                "status" => "completed",
                "budget_type" => "fixed_price",
                "pct" => 100.0,
    ...
    private function seedScoringAndTemplates(): void
    {
        ...
        DB::table("scoring_models")->insert([
            ...
            "description" =>
                "Price + delivery + technical compliance (petrochemical procurement).",
            "type" => "weighted",
            "config" => json_encode(
                [
                    "version" => 2,
                    "criteria" => ["price", "delivery", "technical"],
                ],
                JSON_THROW_ON_ERROR,
            ),
            "status" => "active",
        ...
        DB::table("scoring_policies")->insert([
            ...
            "weights" => json_encode(
                ["price" => 0.45, "delivery" => 0.25, "technical" => 0.3],
                JSON_THROW_ON_ERROR,
            ),
            "status" => "active",
            "created_at" => $this->now,
        ...
        foreach (
            ["Valve & rotating packages", "Chemicals & catalysts"]
            as $name
        ) {
    ...
    private function seedVendors(): void
    {
            ...
            ->sequence(
                fn($sequence) => [
                    ...
                    "country_of_registration" =>
                        $allCountries[$sequence->index % count($allCountries)],
                    "primary_contact_name" => $this->vendorContactName(
                        $sequence->index,
                    ),
                    "primary_contact_email" =>
                        strtolower(
                            preg_replace(
                                "/[^a-z0-9]+/i",
                                ".",
                                $vendorNames[$sequence->index],
                            ),
                        ) . "@vendor.test",
                    "primary_contact_phone" =>
                        "+47 " . (22000000 + $sequence->index * 137),
                    "onboarded_at" => $this->now
                        ->copy()
                        ->subDays(420 - ($sequence->index % 240)),
                ],
            )
            ->create();
        ...
        foreach ($vendorObjects as $index => $vendor) {
            ...
            if ($status === "approved") {
                $vendor->update([
                    "approved_by_user_id" =>
                        $this->userIds[$index % count($this->userIds)],
                    "approved_at" => $this->now
                        ->copy()
                        ->subDays(180 - ($index % 90)),
                    "approval_note" =>
                        "Seeded approval after petrochemical supplier qualification review.",
                ]);
            }
    ...
    private function seedRfqsAndQuotes(): void
    {
        ...
        for ($i = 0; $i < $rfqCount; $i++) {
            ...
            $projectId =
                $i % 11 === 0
                    ? null
                    : $this->projectIds[$i % count($this->projectIds)];
            ...
            $ev =
                25000.0 +
                $i * 17500 +
                (sin($i * 9999) * 10000 - floor(sin($i * 9999) * 10000)) *
                    420000;
            $savings =
                2.5 +
                (sin(($i + 5) * 9999) * 10000 -
                    floor(sin(($i + 5) * 9999) * 10000)) *
                    16.0;
            ...
            $rfq = Rfq::query()->create([
                ...
                "title" =>
                    ...
                    ($i + 1 > count($titles) ? " (lot " . ($i + 1) . ")" : ""),
                "description" =>
                    "Nordfjord Process Chemicals AS — seeded sourcing event (petrochemical maintenance & projects).",
                ...
                RfqLineItem::query()->create([
                    ...
                    "unit_price" => round(
                        120.0 +
                            $j * 37.5 +
                            (sin(($j + $i) * 9999) * 10000 -
                                floor(sin(($j + $i) * 9999) * 10000)) *
                                800,
                        2,
                    ),
                    "currency" => "USD",
                    "specifications" =>
                        "ASME / PED applicable; material cert 3.1 required.",
                    "sort_order" => $j,
                ]);
            }
    ...
    private function seedInvitationsAndQuotes(
        Rfq $rfq,
        int $index,
        string $kind,
    ): void {
        ...
        $approvedVendors = array_filter(
            $this->vendors,
            fn($v) => $v->status === "approved",
        );
        ...
        for ($v = 0; $v < $inviteTotal; $v++) {
            $vend =
                $approvedVendors[($v + $index * 7) % count($approvedVendors)] ??
                $approvedVendors[0];
    ...
    private function seedQuote(
        ...
        if (
            in_array(
                $status,
                ["extracted", "normalizing", "ready", "needs_review", "failed"],
                true,
            )
        ) {
        ...
        if ($status === "ready") {
            $confidence = round(
                72.0 +
                    (sin(($rfqIndex + $quoteIndex) * 9999) * 10000 -
                        floor(sin(($rfqIndex + $quoteIndex) * 9999) * 10000)) *
                        27.0,
                2,
            );
        ...
        if ($status === "failed") {
            ...
            $errorMessage =
                "Unable to parse document structure. Retry exhaustion.";
            $retryCount = 3;
            $errorsCount = 2;
        }
    ...
    private function seedComparisonApprovalAward(
        ...
        foreach ($quotes->values() as $idx => $q) {
            $total =
                58 +
                (($idx * 11) % 35) +
                (sin(($index + $idx) * 9999) * 10000 -
                    floor(sin(($index + $idx) * 9999) * 10000)) *
                    8;
        ...
        $approval = Approval::query()->create([
            ...
            "level" => 1,
            "notes" => $approved
                ? "Award committee sign-off (seed)."
                : "Awaiting delegated approver (seed).",
        ...
        if ($approved) {
            usort(
                $scores,
                static fn(array $a, array $b): int => $b["total_score"] <=>
                    $a["total_score"],
            );
            $winner = $scores[0];
            $winnerQuote = $quotes->first(
                fn($q) => (string) $q->vendor_id ===
                    (string) $winner["vendor_id"],
            );

            if ($winnerQuote !== null) {
                $amount = (float) round(
                    (float) $rfq->estimated_value *
                        (1 - (float) $rfq->savings_percentage / 100),
                    2,
                );
