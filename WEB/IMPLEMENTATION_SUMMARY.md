...
| **Quote Intake** | ✅ | Quote list, detail, normalize, comparison freeze, award, and approvals screens now consume live API data when `NEXT_PUBLIC_USE_MOCKS=false`. Live quote rows preserve nullable fields, normalize confidence/blocks strictly, scope normalization source lines to the active `quoteId`, and expose accessible controlled selection controls on normalization rows. Mock/demo branches remain available for local seed mode. |
