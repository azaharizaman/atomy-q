...
final class InsightAiAvailabilityAdapter implements AiAvailabilityPortInterface
{
    ...
    public function __construct(private readonly AiRuntimeStatusInterface $runtimeStatus)
    {
    }
    ...
    {
        try {
            $status = $this->runtimeStatus->snapshot()->capabilityStatuses[$featureKey] ?? null;
        ...
        $this->reasonCodeCache[$featureKey] = $status?->reasonCodes ?? ['ai_capability_not_configured'];
    ...
    public function reasonCodes(string $featureKey): array
    {
        ...
        return $this->reasonCodeCache[$featureKey] ?? ['ai_capability_not_configured'];
    }
}
