...
final readonly class DashboardFactsAdapter implements DashboardFactsPortInterface
{
    public function factsForTenant(string $tenantId): DashboardFactsDto
    {
        ...
        return new DashboardFactsDto(
            metrics: [
                ...
                new MetricFactDto('total_savings', $this->totalSavings($tenantId)),
    ...
    private function tenantRfqQuery(string $tenantId): \Illuminate\Database\Eloquent\Builder
    {
    ...
    {
        $rfqs = $this->tenantRfqQuery($tenantId)
    ...
    private function averageCycleTime(string $tenantId): MetricFactDto
    {
        if (! Schema::hasColumn('awards', 'signoff_at')) {
            return new MetricFactDto('avg_cycle_time_days', null, 'not_available', 'source_domain_not_implemented');
        }
        ...
        return new MetricFactDto('avg_cycle_time_days', round($totalDays / max($awards->count(), 1), 2));
    }
    ...
    private function riskAlerts(string $tenantId): array
    {
        return VendorFinding::query()
            ...
            ->map(static fn (VendorFinding $finding): array => [
