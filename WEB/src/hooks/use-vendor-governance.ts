...
import {
    normalizeAiNarrativePayload,
    type AiNarrativeSummary,
} from "@/hooks/use-ai-narrative-summary";
...
function stringList(value: unknown, field: string, context: string): string[] {
    ...
    return value.map((item, index) =>
        requireText(item, `${field}.${index}`, context),
    );
}
...
function normalizeEvidence(
    row: unknown,
    index: number,
): VendorGovernanceEvidence {
    if (!isObject(row)) {
        throw new Error(
            `Invalid vendor governance evidence at index ${index}: expected object`,
        );
    }
    ...
    return {
        ...
        vendorId: requireText(
            pickField(row, "vendor_id", "vendorId"),
            "vendor_id",
            context,
        ),
        ...
        observedAt: nullableText(
            pickField(row, "observed_at", "observedAt"),
            "observed_at",
            context,
        ),
        expiresAt: nullableText(
            pickField(row, "expires_at", "expiresAt"),
            "expires_at",
            context,
        ),
        reviewStatus: requireText(
            pickField(row, "review_status", "reviewStatus"),
            "review_status",
            context,
        ),
        reviewedBy: nullableText(
            pickField(row, "reviewed_by", "reviewedBy"),
            "reviewed_by",
            context,
        ),
...
function normalizeFinding(
    row: unknown,
    index: number,
): VendorGovernanceFinding {
    if (!isObject(row)) {
        throw new Error(
            `Invalid vendor governance finding at index ${index}: expected object`,
        );
    }
    ...
    return {
        ...
        domain: requireText(pickField(row, "domain"), "domain", context),
        issueType: requireText(
            pickField(row, "issue_type", "issueType"),
            "issue_type",
            context,
        ),
        severity: requireText(pickField(row, "severity"), "severity", context),
        status: requireText(pickField(row, "status"), "status", context),
        openedAt: nullableText(
            pickField(row, "opened_at", "openedAt"),
            "opened_at",
            context,
        ),
        openedBy: nullableText(
            pickField(row, "opened_by", "openedBy"),
            "opened_by",
            context,
        ),
...
export function normalizeVendorGovernancePayload(
    payload: unknown,
): VendorGovernanceSummary {
    const envelope = isObject(payload) ? payload : null;
    const data =
        envelope !== null && isObject(envelope.data) ? envelope.data : null;
    if (data === null) {
        throw new Error(
            "Invalid vendor governance payload: expected object envelope with data object.",
        );
    }
    ...
    if (
        ...
        !isObject(scores) ||
        !Array.isArray(warningFlags)
    ) {
        throw new Error(
            "Invalid vendor governance payload: expected evidence, findings, summary_scores, and warning_flags.",
        );
    }
    ...
    let narrative = null;
    if (
        isObject(aiNarrativeField) &&
        (typeof aiNarrativeField.feature_key === "string" ||
            typeof aiNarrativeField.featureKey === "string")
    ) {
        try {
    ...
    return {
        vendorId: requireText(
            pickField(data, "vendor_id", "vendorId"),
            "vendor_id",
        ...
        scores: {
            esgScore: requireNumber(
                pickField(scores, "esg_score", "esgScore"),
                "esg_score",
                "Invalid vendor governance payload",
            ),
...
async function fetchVendorGovernance(
    vendorId: string,
): Promise<VendorGovernanceSummary> {
    const response = await api.get(
        `/vendors/${encodeURIComponent(vendorId)}/governance`,
    );
...
export function useGenerateVendorGovernanceNarrative(vendorId: string) {
    ...
    return useMutation({
        mutationFn: async (id?: string): Promise<VendorGovernanceSummary> => {
            const targetId = id?.trim() || normalizedVendorId;
            if (targetId === "") {
                throw new Error(
                    "Vendor id is required to generate governance narrative.",
                );
            }

            const response = await api.post(
                `/vendors/${encodeURIComponent(targetId)}/governance/generate`,
            );

            return normalizeVendorGovernancePayload(response.data);
        },
        onSuccess: (summary, variables) => {
            const targetId = variables?.trim() || normalizedVendorId;
            queryClient.setQueryData(
                ["vendors", targetId, "governance"],
                summary,
            );
        },
    });
...
export function useVendorGovernanceMap(vendorIds: string[]) {
    const uniqueVendorIds = Array.from(
        new Set(vendorIds.map((id) => id.trim()).filter(Boolean)),
    ).sort();
