...
import type { ConfidenceVariant, SLAVariant, StatusVariant } from './tokens';
...
const STATUS_MAP: Record<StatusVariant, StatusConfig> = {
  active: { label: 'Active', className: 'bg-green-50 text-green-700 border border-green-200', dot: true },
  approved: { label: 'Approved', className: 'bg-green-50 text-green-700 border border-green-200', icon: <CheckCircle2 size={11} /> },
  paid: { label: 'Paid', className: 'bg-green-50 text-green-700 border border-green-200', icon: <CheckCheck size={11} /> },
  final: { label: 'Final', className: 'bg-green-50 text-green-700 border border-green-200', icon: <CheckCircle2 size={11} /> },
  generated: { label: 'Generated', className: 'bg-blue-50 text-blue-700 border border-blue-200', dot: true },
  awarded: { label: 'Awarded', className: 'bg-indigo-50 text-indigo-700 border border-indigo-200', dot: true },
  pending: { label: 'Pending', className: 'bg-amber-50 text-amber-700 border border-amber-200', icon: <Clock size={11} /> },
  processing: { label: 'Processing', className: 'bg-amber-50 text-amber-700 border border-amber-200', icon: <Loader2 size={11} className="animate-spin" /> },
  preview: { label: 'Preview', className: 'bg-amber-50 text-amber-700 border border-amber-200', icon: <Eye size={11} /> },
  stale: { label: 'Stale', className: 'bg-amber-50 text-amber-700 border border-amber-200', icon: <RefreshCw size={11} /> },
  due: { label: 'Due', className: 'bg-amber-50 text-amber-700 border border-amber-200', icon: <AlertTriangle size={11} /> },
  publishing: { label: 'Publishing', className: 'bg-amber-50 text-amber-700 border border-amber-200', icon: <Loader2 size={11} className="animate-spin" /> },
  rejected: { label: 'Rejected', className: 'bg-red-50 text-red-700 border border-red-200', icon: <XCircle size={11} /> },
...
export function StatusBadge({ status, label, size = 'sm', className = '' }: StatusBadgeProps) {
  ...
  return (
    <span
      ...
      className={['inline-flex items-center rounded-full font-medium', sizeClass, config.className, className].join(' ')}
    >
      {config.dot && <span aria-hidden="true" className={['w-1.5 h-1.5 rounded-full', getDotColor(status)].join(' ')} />}
      {config.icon && !config.dot && <span aria-hidden="true">{config.icon}</span>}
      {accessibleLabel}
    </span>
  );
}
...
export function SLATimerBadge({ variant, value, className = '' }: SLATimerBadgeProps) {
  return (
    <span className={['inline-flex items-center gap-1 rounded-full text-xs font-medium px-2 py-0.5 h-5 tabular-nums', SLA_STYLES[variant], className].join(' ')}>
      <Clock size={10} />
      {value}
    </span>
  );
    ...
    return (
      <div className={['flex items-center gap-2', className].join(' ')}>
        <div className="flex-1 h-1.5 rounded-full bg-slate-200 overflow-hidden">
          <div className={['h-full rounded-full', config.bar].join(' ')} style={{ width: `${clampedPercentage}%` }} />
        </div>
...
export function VersionChip({ version, className = '' }: VersionChipProps) {
  return (
    <span className={['inline-flex items-center rounded px-1.5 py-0.5 text-[11px] font-medium bg-slate-100 text-slate-600 border border-slate-200 font-mono', className].join(' ')}>
      {version}
    </span>
  );
}
...
export function CountBadge({ count, variant = 'default', className = '' }: CountBadgeProps) {
  ...
  return (
    <span className={['inline-flex items-center justify-center min-w-[18px] h-[18px] rounded-full text-[10px] font-medium px-1', variantStyles[variant], className].join(' ')}>
      {count}
    </span>
  );
}
...
export function StatusDot({ color, className = '' }: StatusDotProps) {
  return <span className={['inline-block w-2 h-2 rounded-full', DOT_COLORS[color], className].join(' ')} />;
}
